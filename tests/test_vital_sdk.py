"""
Tests for canela-molida's vital-sdk integration (VitalClient + VitalConfig).
...
def mock_httpx_response_ok():
    ...
    response.json.return_value = {"status": "ok", "event_id": "evt-99999"}
...
def mock_httpx_client(mock_httpx_response_ok):
    ...
    with patch("app.integrations.vital_sdk.client.httpx.AsyncClient") as MockClientClass:
...
class TestVitalConfig:
    """Tests for VitalConfig defaults and environment variable overrides."""

    def test_defaults(self):
        from app.integrations.vital_sdk.config import VitalConfig

        config = VitalConfig()
        assert config.core_url == "http://localhost:8888"
        assert config.api_key == ""
        assert config.service_name == "canela-molida"
        assert config.service_port == 3690
    ...
    def test_env_override_service_name(self, monkeypatch):
        monkeypatch.setenv("VITAL_SERVICE_NAME", "custom-service")
        from app.integrations.vital_sdk.config import VitalConfig
    ...
    def test_env_override_api_key(self, monkeypatch):
        monkeypatch.setenv("VITAL_API_KEY", "secret-key-123")
        from app.integrations.vital_sdk.config import VitalConfig
    ...
    def test_env_override_service_port(self, monkeypatch):
        monkeypatch.setenv("VITAL_SERVICE_PORT", "4000")
        from app.integrations.vital_sdk.config import VitalConfig
    ...
    def test_env_override_redis_url(self, monkeypatch):
        monkeypatch.setenv("VITAL_REDIS_URL", "redis://custom-redis:6380")
        from app.integrations.vital_sdk.config import VitalConfig
...
class TestVitalClientInstantiation:
    ...
    def test_instantiation_with_default_config(self, mock_httpx_client):
        from app.integrations.vital_sdk.client import VitalClient
        from app.integrations.vital_sdk.config import VitalConfig

        config = VitalConfig()
        client = VitalClient(config=config)

        assert client.config.service_name == "canela-molida"
        assert client.config.service_port == 3690
...
class TestLifecycle:
    ...
    async def test_connect_calls_health_check(self, mock_httpx_client):
        ...
        # health_check does a GET to /api/v1/health
...
class TestRegister:
    ...
    @pytest.mark.asyncio
    async def test_register_sends_system_event(self, mock_httpx_client):
        from app.integrations.vital_sdk.client import VitalClient
        ...
        assert body["source"] == "canela-molida"
        assert body["event_type"] == "system"
        assert body["payload"]["service_name"] == "canela-molida"
        assert body["payload"]["service_port"] == 3690
...
class TestPublishEvent:
    ...
    @pytest.mark.asyncio
    async def test_publish_event_with_all_params(self, mock_httpx_client):
        from app.integrations.vital_sdk.client import VitalClient
        ...
        result = await client.publish_event(
            ...
            payload={"lesson_id": "L42", "score": 95},
            event_type="course",
        ...
        assert body["action"] == "lesson.completed"
        assert body["source"] == "canela-molida"
        assert body["event_type"] == "course"
        assert body["payload"] == {"lesson_id": "L42", "score": 95}
        assert body["metadata"] == {"platform": "web"}
        assert body["tags"] == ["gamification"]
        assert body["subcategory"] == "interactive"
    ...
    @pytest.mark.asyncio
    async def test_publish_event_various_categories(self, mock_httpx_client):
        from app.integrations.vital_sdk.client import VitalClient

        client = VitalClient()
        await client.connect()

        for category in ["health", "education", "security", "system"]:
            mock_httpx_client.post.reset_mock()
            result = await client.publish_event(
                category=category,
                action="test.action",
...
class TestHealthCheck:
    ...
    async def test_health_check_returns_response(self, mock_httpx_client):
        ...
        client = VitalClient()
        # Don't call connect() to avoid triggering health_check automatically
        # Instead set up the http client directly
        client._http = mock_httpx_client

        result = await client.health_check()

        mock_httpx_client.get.assert_called_with("/api/v1/health")
        assert result == {"status": "ok", "event_id": "evt-99999"}
...
class TestCreateEvent:
    ...
    def test_create_event_with_all_params(self):
        from app.integrations.vital_sdk.events import create_event

        event = create_event(
            category="health",
            action="vitals.recorded",
            source="canela-molida",
            event_type="biometric",
            payload={"heart_rate": 75},
            metadata={"device": "sensor"},
            tags=["realtime"],
            subcategory="cardiac",
        )

        assert event["category"] == "health"
        assert event["action"] == "vitals.recorded"
        assert event["source"] == "canela-molida"
        assert event["event_type"] == "biometric"
        assert event["payload"] == {"heart_rate": 75}
        assert event["metadata"] == {"device": "sensor"}
        assert event["tags"] == ["realtime"]
        assert event["subcategory"] == "cardiac"
...
class TestEventConstants:
    """Tests for EventCategory and EventBusChannel constants."""

    def test_event_category_constants(self):
        from app.integrations.vital_sdk.events import EventCategory
    ...
    def test_event_bus_channel_constants(self):
        from app.integrations.vital_sdk.events import EventBusChannel
    ...
    def test_event_bus_channel_for_category(self):
        ...
        assert EventBusChannel.for_category("health") == "vital.health"
...
class TestGracefulDegradation:
    ...
    async def test_connect_survives_health_check_failure(self, mock_httpx_client):
        ...
        # Heartbeat task should NOT be started since connect failed
        # (depends on implementation: the code catches the exception)
        await client.disconnect()
...
class TestModuleExports:
    ...
    def test_public_exports(self):
        from app.integrations.vital_sdk import (
            VitalClient,
            VitalConfig,
            EventCategory,
            EventBusChannel,
